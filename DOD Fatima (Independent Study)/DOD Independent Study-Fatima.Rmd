...
# d <- committees %<>% filter(agency == "DOD")
# 
# load("data/committees.RData")
# dcommittees <- committees %>% filter(str_detect(committees, "ARMED SERVICES|VETERANS|FOREIGN RELATIONS|APPROPRIATIONS")) 
# 
...
d %>% filter(female==1) %>% count(bioname,party,chamber,cqlabel,committees, TYPE, sort = T) %>% top_n(10, n) %>% kable(caption = "Female Legislators who wrote the most letters to DOD")

d %>% filter(female==1) %>% filter(str_detect(committees, "ARMED SERVICES|VETERANS|FOREIGN RELATIONS|APPROPRIATION")) %>% count(bioname,party,chamber,cqlabel,committees, TYPE, sort = T) %>% top_n(10, n) %>% kable(caption = "Female Relevant Committee Members who wrote the most letters to DOD according")
...
In the future, I hope to continue my exploration with this topic by analyzing some different aspects to help evaluate the overlying question of how Department of Defense agencies interact with constituents and legislators.
