...
git pull
Rscript -e 'library(rmarkdown);rmarkdown::render("DOD Fatima (Independent Study).Rmd", "html_document")'
...
```{r, include=FALSE}
  ...
  #source(here("setup.R")) ##we want to do source.setup here right??
```
...
dcommittees %>%  
  ...
  distinct() %>% 
  filter(str_detect(committee, "DEFENSE$|VETERAN$")) %>% 
  ...
  labs(title = paste("Letters to DOD and VA from Committee Leadership"),
       y = "", 
       x = "",
...
## Many letters to DOD/VA are cosigned
...
DOD_letters %>% 
  filter(year>2006, ##range of 2007-2017 is what I want
         year<2018) %>% 
  group_by(ID) %>% 
  top_n(1) %>% 
...
```{r cosigned_coded}
DOD_letters %>% 
  filter(!is.na(), #what does Freelancer mean
         year>2006,
         year<2018) %>% 
...
## Edit this to make it more relevant
