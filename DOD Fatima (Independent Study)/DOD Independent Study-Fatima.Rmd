...
d %>%  
  mutate(chair = str_remove(chair,"[0-9]* |^NA "),
         committees = paste(chamber, committees))%>% 
  dplyr::select(committees, position, chair, DATE) %>% 
  distinct() %>% 
  filter(str_detect(committees, "ARMED SERVICES")) %>% 
  mutate(position = ifelse(position == "Other", NA, position)) %>%
  #filter(# !is.na(position),chair_since_2007 == T) %>%
  drop_na(chair) %>% 
  dplyr::select(DATE, chair, position, committees) %>% 
  distinct() %>% 
  ggplot() +
  geom_point(
    aes(x = DATE, 
  ...
  facet_grid(committees ~ ., scales = "free_y", space = "free_y", switch = "both") 
```
...
d %>% filter(female==1) %>% count(bioname,party,chamber,cqlabel,committees, sort = T) %>% top_n(10, n) %>% kable(caption = "Legislators who wrote the most letters to DOD")


a %>% filter(female==1) %>% count(bioname,party,chamber,cqlabel,committees, sort = T) %>% top_n(10, n) %>% kable(caption = "Legislators who wrote the most letters to DOD")
