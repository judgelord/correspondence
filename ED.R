...
clean <- function(file.name) {
  ...
  data%<>%
  mutate(TYPE = ifelse (!grepl("[0-9]", TYPE) & grepl("STUDENT LOAN QUESTION|STATUS CHECK|CONSTITUENT|CASEWORK|STUDENT LOAN REPAYMENT|FSA QUESTION|STUDENT LOAN INQUIRY|FOIA REQUEST|STUDENT LOAN DISCHARGE|STAFFER.*QUESTION|STAFFER.*REQUEST|FINANCIAL AID|LOAN FORGIVENESS|DISCHARGE APPLICATION|DEFAULTED STUDENT LOAN|SEEKING ASSISTANCE|REQUEST ASSISTANCE|HIS APPLICATION|STUDENT LOAN|HE OWES|STATUS UPDATE|REGARDING HIS|SHE IS|FSA2|FSA INFORMATION|NEED ASSISTANCE|HIGH SCHOOL|HER SON|HER DAUGHTER|HIS SON|HIS DAUGHTER|FAFSA", SUBJECT, ignore.case = TRUE), "1", TYPE)) %>%
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("STUDENT LOAN QUESTION|STATUS CHECK|CONSTITUENT|CASEWORK|STUDENT LOAN REPAYMENT|FSA QUESTION|STUDENT LOAN INQUIRY|FOIA REQUEST|STUDENT LOAN DISCHARGE|STAFFER.*QUESTION|STAFFER.*REQUEST|FINANCIAL AID|LOAN FORGIVENESS|DISCHARGE APPLICATION|DEFAULTED STUDENT LOAN|SEEKING ASSISTANCE|REQUEST ASSISTANCE|HIS APPLICATION|STUDENT LOAN|HE OWES|STATUS UPDATE|REGARDING HIS|SHE IS|FSA2|FSA INFORMATION|NEED ASSISTANCE|HIGH SCHOOL|HER SON|HER DAUGHTER|HIS SON|HIS DAUGHTER|FAFSA", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
  mutate(TYPE = ifelse (!grepl("[0-9]", ALT_TYPE) & grepl("LANGSTON UNIVERSITY|COMMUNITY COLLEGE|TEACHING AMERICAN HISTORY|INVESTING IN INNOVATION|RACE TO THE TOP|IMPROVEMENT OF POSTSECONDARY|BEHALF.*UNIVERSITY|CAROL M. WHITE|PROMISE NEIGHBORHOOD|PROGRAM APPLICATION|ON BEHALF OF.*GRANT APPLICATION|GRANT APPLICATION", SUBJECT, ignore.case = TRUE), "3", TYPE)) %>%
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("LANGSTON UNIVERSITY|COMMUNITY COLLEGE|TEACHING AMERICAN HISTORY|INVESTING IN INNOVATION|RACE TO THE TOP|IMPROVEMENT OF POSTSECONDARY|BEHALF.*UNIVERSITY|CAROL M. WHITE|PROMISE NEIGHBORHOOD|PROGRAM APPLICATION|ON BEHALF OF.*GRANT APPLICATION|GRANT APPLICATION", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
