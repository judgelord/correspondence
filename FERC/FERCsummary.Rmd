...
Non_Constituent_money <- constituents_money %>% filter(industry == "energy", party_name != "Independent") 
...
# Models
```{r mplot_function}
letters_money %<>% mutate(party_name != "Independent")
constituents_money %<>% mutate(party_name != "Independent")
...
mAll <- glm(total_letters ~ PAC_100k, 
            data = letters_money %>% filter(industry == "energy") %>% select(icpsr,party_name, congress, PAC_100k, total_letters) %>% distinct(), 
         family = poisson)  
# tidy(mAll)
mAll %>% mplot()+
...
Non_Constituent_money %>%
                         filter(industry == "energy") %>% 
                         select(icpsr, congress, party_name,PAC_100k, Non_Constituent_Letters)%>% 
...
mNonConstituentLog <- lm(Non_Constituent_Letters ~ Log_PAC_contributions, 
                          data = Non_Constituent_money %>% 
                            filter(industry == "energy") %>% 
                            select(icpsr, congress, party_name,PAC_contributions, Non_Constituent_Letters)%>%
                            distinct() %>% 
...
mPro <- glm(ProBusiness_Letters ~ PAC_100k,
            data = letters_money %>% 
              filter(industry == "energy") %>% 
              select(icpsr, congress, party_name,PAC_100k, ProBusiness_Letters) %>% 
              distinct(),
            family = poisson)  
...
mProLog <- glm(ProBusiness_Letters ~ Log_PAC_contributions, 
              data = letters_money %>% 
              filter(industry == "energy") %>% 
              select(icpsr, congress,party_name, Log_PAC_contributions, ProBusiness_Letters) %>% 
              distinct(),
            family = poisson) 
mProLog %>% mplot()+
...
letters_money %>% 
                        ...
                        select(icpsr, congress, PAC_100k, party_name, AntiBusiness_Letters, letter_position) %>%
...
mProTotal <- glm(ProBusiness_Letters_Per_Cycle ~ PAC_100k_total, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr,congress,party_name, PAC_contributions_total, ProBusiness_Letters_Per_Cycle) %>%
...
letters_money %>%
                        filter(industry == "energy") %>%
                        filter(party_name != "Independent") %>%
                        select(icpsr, PAC_100k, congress,party_name, AntiBusiness_Letters, letter_position) %>%
                      distinct()
...
mAnti <- glm(AntiBusiness_Letters ~ PAC_100k, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress,party_name, PAC_100k, AntiBusiness_Letters) %>%
                      distinct(),
            family = poisson)
# tidy(mAnti)
mAnti %>% mplot()+
...
mAntiLog <- glm(AntiBusiness_Letters ~ Log_PAC_contributions, 
                        ...
                        filter(industry == "energy") %>% 
                        # filter(party_name != "Independent") %>% 
                        select(icpsr,congress,party_name, Log_PAC_contributions, AntiBusiness_Letters) %>%
                      distinct(),
            family = poisson)
# tidy(mAntiLog)
mAntiLog %>% mplot()+
...
mAntiParty <- glm(AntiBusiness_Letters ~ party_name,
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress,party_name, PAC_100k, AntiBusiness_Letters, party_name) %>%
                      distinct(),
            family = poisson)
# tidy(mAntiParty)
mplot(mAntiParty) +
...
mAntiTotal <- glm(AntiBusiness_Letters_Per_Cycle ~ PAC_100k_total, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress,party_name,PAC_contributions_total, AntiBusiness_Letters_Per_Cycle) %>%
                   mutate(PAC_100k_total = PAC_contributions_total/100000) %>% 
                      distinct(),
            family = poisson)
