...
```{r senate_percapita_map, fig.width=3}
Chamber = "Senate"
...
```{r members_barcoded, fig.height=2.2, fig.width=3}
...
```{r pac_amounts, fig.height= 3.6, fig.width=6}
library(tidyverse)
...
letters_money %>% 
  ggplot()  + 
  aes(x = PAC_contributions, y = total_letters) +
  geom_jitter() +
  geom_smooth() + 
...
letters_money %>% 
  ggplot()  + 
  aes(x = PAC_contributions, y = ProBusiness_Letters) +
  geom_jitter() +
  geom_smooth() + 
  facet_wrap("party_name" )
...
letters_money %>% 
  ggplot()  + 
  aes(x = PAC_contributions, y = AntiBusiness_Letters) +
  geom_jitter() +
  geom_smooth() + 
  facet_wrap("party_name" )
...
mAnti_Full <- glm(AntiBusiness_Letters ~ PAC_100k + party_name, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                    mutate(PAC_100k = Log_PAC_contributions) %>% 
...
write_lines(
  stargazer(mAll, mPro, mAnti, mConstituent, 
            covariate.labels = c("Campaign Contributions", "Republican"),
...
write_lines(stargazer(mAll, mPro, mAnti, mNonConstituent, 
            covariate.labels = c("Campaign Contributions", "Republican"),
          dep.var.caption  = "Letters to FERC",
          dep.var.labels   = c("Total", "Pro-business", "Anti-business", "Non-constituent"),
            float=F),
...
write_lines(
  stargazer::stargazer(mProTotal, mAntiTotal,             covariate.labels = c("Campaign Contributions", "Republican"),
          dep.var.caption  = "Letters to FERC",
          dep.var.labels   = c("Total", "Pro-business", "Anti-business", "Constituent"),
          float=F),
...
# All models 
write_lines(
  stargazer::stargazer(mAll, mPro, mAnti,mNonConstituent, mProLog, mAntiLog, mProParty, mProTotal, mAntiTotal, mAntiParty, covariate.labels = c("Campaign Contributions", "Republican"),
          dep.var.caption  = "Letters to FERC",
          dep.var.labels   = c("Total", "Pro-business", "Anti-business", "Constituent"),
          float=F),
...
write_lines(
  stargazer::stargazer(mAll_Full, mAll_Fullx, mPro_Full, mAnti_Full,mPro_Fullx, mAnti_Fullx,            covariate.labels = c("Campaign Contributions", "Republican"),
          dep.var.caption  = "Letters to FERC",
          dep.var.labels   = c("Total", "Pro-business", "Anti-business", "Constituent"), float=F),
