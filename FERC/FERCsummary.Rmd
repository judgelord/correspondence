...
FERC_letters %<>% 
  select(ID, SUBJECT, Notes, #last_name, 
         Freelancer, 
...
# FERC_letters %>% filter(ID %in% d$ID)
...
<!-- we identified Members of Congress in `r FERC_letters %>% filter(!is.na(last_name)) %>% select(Notes) %>% distinct() %>% nrow()`-->
...
Using CRP industry codes for PACs and DIME contribution matrix, sum up sector-level contributions by a member per cycle, matching a congress with the cycle when it was elected (e.g., 116th Congress = 2018 cycle). 
...
members %<>%  filter(congress >105) %>% filter(chamber %in% c("House", "Sentate")) 

pac_amounts %>% count(icpsr, congress, industry) %>% filter(n >1)
members %>% count(icpsr, congress) %>% filter(n >1)

pac_amounts %<>% left_join(members %>% select(congress, icpsr, bioname) %>% distinct() )  %>% distinct()
...
mNonConstituent <- lm(Non_Constituent_Letters ~ PAC_Millions, 
...
# FIXME - why not poisson?
mNonConstituentLog <- lm(Non_Constituent_Letters ~ Log_PAC_contributions, 
                          data = Non_Constituent_money %>% 
                            filter(industry == "energy") %>% 
                            select(icpsr, congress, PAC_contributions, Non_Constituent_Letters)%>%
                            distinct() %>% 
                            filter(!is.na(PAC_contributions)) %>% #FIXME why is this ever NA
                            mutate(Log_PAC_contributions = log(PAC_contributions+1))) 
# tidy(mNonConstituent)
