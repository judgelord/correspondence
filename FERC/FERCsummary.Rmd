...
d %>%   filter(str_detect(SUBJECT, "forwards")) %>% add_count(bioname, sort = T) %>% top_n(10, n) %>% select(bioname, SUBJECT) %>% kable(caption = "Legislators who forward the most letters")
...
d %>% 
  ...
  mutate(percent = n / sum(n) * 100 ) %>% 
  ggplot() + 
  aes(y = percent, x = congress, fill = For) + 
  geom_col(position = "stack") + 
  #geom_text(aes(label = n)) +
  facet_grid(majority ~ party_name) + 
  scale_fill_viridis_d(option = "C", end = .8) + 
...
d %>% 
  count(party_name, letter_position, majority, congress) %>% 
    filter(party_name != "Independent",
         ...
         !is.na(letter_position),
         !is.na(majority)) %>% 
  group_by(party_name, majority, congress) %>% 
  mutate(percent = n / sum(n) * 100 ) %>% 
  ggplot() + 
  aes(y = percent, x = factor(congress), fill = letter_position) + 
  geom_col(position = "stack") + 
  facet_grid(majority ~ party_name)+ 
  scale_fill_viridis_d(option = "C", end = .8) + 
  labs(x = "",
...
d %>% 
  ...
  scale_x_continuous(breaks = NULL) +
  scale_y_continuous(breaks = NULL) +
  labs(x = "", y = "", title = paste("Contacts Per thousand Residents from Members of the", Chamber)) +
  scale_fill_viridis_c(option = "C", end = .8) +
...
pac_amounts %>%
  ...
  labs(y = "Total Industry PAC contributions (hundreds of thousands)", 
...
pac_amounts %>%
  ...
  geom_col(position = "dodge") + 
  facet_wrap("industry", scales = "free") +
  labs(y = "Average Energy-sector PAC contributions per member (hundreds of thousands) \n (ommitting energy extraction companies)", 
       x = "FEC Electoral Cycle")
...
## Outliers other than Virgil H. Goode, Jr. for the 107th
```{r outlier}
letters_money %>%
...
mAll <- glm(total_letters ~ PAC_100k, 
            data = letters_money %>% filter(industry == "energy") %>% select(icpsr, congress, PAC_100k, total_letters) %>% distinct(), 
         family = poisson)  
# tidy(mAll)
mAll %>% mplot()+
  labs(x="", y="Total additional letters per congress per thousand dollars")
```

## Non-Constituent Letters
```{r non-constituent models, fig.height=1}
Non_Constituent_money %<>% mutate(PAC_100k = PAC_contributions/100000)

mNonConstituent <- lm(Non_Constituent_Letters ~ PAC_100k, 
                       data = Non_Constituent_money %>%
                         filter(industry == "energy") %>% 
                         select(icpsr, congress, PAC_100k, Non_Constituent_Letters)%>% 
                         distinct() ) 
# tidy(mNonConstituent)
mNonConstituent %>% mplot()+
  labs(x="", y="Additional non-constituent letters per congress per thousand dollars")
...
mNonConstituentLog %>% mplot()+
  labs(x="", y="Additional non-constituent letters per congress per logged thousand dollars")
```
...
mPro <- glm(ProBusiness_Letters ~ PAC_100k,
            data = letters_money %>% 
              filter(industry == "energy") %>% 
              select(icpsr, congress, PAC_100k, ProBusiness_Letters) %>% 
              distinct(),
            family = poisson)  

mPro %>% mplot()+
  labs(x="", y="Additional pro-business letters per congress per thousand dollars")
...
mPro_pac_party <- glm(ProBusiness_Letters ~ PAC_100k + party_name,
                      data = letters_money %>% 
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress, PAC_100k, party_name, ProBusiness_Letters) %>% 
              distinct(),
            family = poisson)
...
mAll_Full <- glm(total_letters ~ PAC_100k + party_name, 
                 data = letters_money %>% 
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress, PAC_100k, party_name, total_letters) %>%
                      distinct(),
            family = poisson)
...
mAnti_Full <- glm(AntiBusiness_Letters ~ PAC_100k + party_name, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress, PAC_100k, party_name, AntiBusiness_Letters) %>%
                      distinct(),
            family = poisson)
...
# tidy(mProTotal)
mplot(mProTotal)+
  labs(x="", y="Average additional letters per cycle per thousand dollars 1990-2014")
```
...
mAnti <- glm(AntiBusiness_Letters ~ PAC_100k, 
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, PAC_100k, AntiBusiness_Letters) %>%
                      distinct(),
            family = poisson)
# tidy(mAnti)
mAnti %>% mplot()+
  labs(x="", y="Additional anti-buisness letters per congress per thousand dollars")
...
mAntiParty <- glm(AntiBusiness_Letters ~ party_name,
                        ...
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, PAC_100k, AntiBusiness_Letters, party_name) %>%
                      distinct(),
            family = poisson)
# tidy(mAntiParty)
mplot(mAntiParty) +
...
dplot(models)+ 
    labs(color = "Dependent Variable",
         shape = "Dependent Variable",
         x = "Additional letters per thousand dollars per cycle")
...
dplot(models) + 
    labs(x = "Average additional letters per cycle per thousand dollars 1990-2014")
