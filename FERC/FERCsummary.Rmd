...
pac_amounts %>% 
  ...
  ggplot() + geom_histogram(aes(x = n)) + facet_grid()
...
pac_amounts$amounts <- pac_amounts$n
pac_amounts$industry <- "energy"
pac_amounts$amounts <- abs(pac_amounts$amounts)
...
letters_money_total <- letters_money %>% 
  ...
  summarise(ProBusiness_Letters_Per_Cycle = mean(ProBusiness_Letters, na.rm = TRUE),
            PAC_contributions = sum(amounts, na.rm = TRUE)) %>% 
  ungroup()
...
Antiletters_money_total <- Antiletters_money %>% 
  # FIXME there should be no NA amounts
  filter(!is.na(amounts)) %>% 
  group_by(icpsr, name_state, party) %>% 
...
All_Letters_money %<>% 
  mutate(Log_PAC_contributions = log(amounts+1))  

mAll <- glm(All_Letters ~ PAC_contributions, data = All_Letters_money %>%
           mutate(PAC_contributions = amounts/1000000), 
         family = poisson)  
tidy(mAll)
mAll %>% mplot()+
...
mPro <- glm(ProBusiness_Letters ~ amounts, data = letters_money %>% mutate(amounts = amounts/1000000), family = poisson)  
tidy(mPro)
mPro %>% mplot()+
...
Non_Constituent_money$PAC_contributions <- Non_Constituent_money$amounts
