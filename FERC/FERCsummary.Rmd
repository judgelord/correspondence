...
pac_amounts %<>% ungroup() %>% group_by(bioname, congress, industry) %>%  top_n(1, PAC_contributions)
...
letters_money <- d %>% 
  ...
  ungroup() %>%
    complete(letter_position, congress, icpsr, 
             fill = list(n = 0))

letters_money %>% arrange(icpsr)

letters_money %<>% left_join(members %>% select(congress, icpsr, bioname) %>% distinct()) %>%
...
# inspect for 0 counts (should be equal across years per industry)
letters_money %>% count(congress, industry, letter_position) %>% arrange(industry)

# compare to memebers 
members%>% left_join(letters_money) %>% ungroup() %>%  count(congress, industry, letter_position ) 

# members that are not in the pac data 
members%>% left_join(letters_money) %>% ungroup() %>% filter(is.na(industry))
