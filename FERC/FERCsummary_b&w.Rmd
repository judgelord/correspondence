...
```{r, include=FALSE}
  ...
  options(
    ...
    ggplot2.discrete.color = "viridis",
...
dwplot <- function(...) dotwhisker::dwplot(...) + 
...
```{r part-constituent, fig.height=2.5, fig.width=4.6}
...
```{r party-constituent, fig.height=2.5, fig.width=6.2}
...
```{r part-business, fig.height=3, fig.width=4.5}
d %>% 
...
full_join( all_percent, d_percent)%>% 
  ...
  labs(x = "",
       fill = "",
       color = "",
       y = "Percent of Legislator Contacts", #paste("Number of Contacts, N =", sum(nrow(all_contacts))),
...
```{r population, fig.height= 7.4, fig.width=6}
...
f<- d %>% 
  ...
  geom_text(aes(x = log(pop2010), 
            ...
            hjust = .4) + 
  ##theme_bw() +
  labs(title = "Senator Requests per Year to FERC Year by State Population",
...
```{r senate_percapita_map, fig.width=7, fig.height=3}
Chamber = "Senate"
...
ggplot() + 
  ...
  labs(x = "", y = "", 
       fill = "Total Contacts",
       title = paste("Total Number of Contacts\nfrom Members of the", Chamber)) +
  scale_fill_viridis_c(option = "C", end = .8) +
  theme(legend.position = "bottom", 
        #legend.title = element_blank(),
        panel.background = element_blank(), 
        plot.title = element_text(hjust = .5),
        panel.border = element_blank() ) # + facet_grid(. ~ Constituent)
...
percap <- d %>% 
  ...
  scale_y_continuous(breaks = NULL) +
  labs(x = "", y = "",
       fill = "Contacts Per Capita",
       title = paste("Contacts Per 100,000 Residents\nfrom Members of the", Chamber)) +
  scale_fill_viridis_c(option = "C", end = .8) +
  theme(legend.position = "bottom", 
        plot.title = element_text(hjust = .5),
        panel.border = element_blank())#+facet_grid(. ~ Constituent)
...
```{r senate_percapita_map-type, fig.width=3.5, fig.height=6}
# By type2
d %>% 
  filter(chamber == Chamber) %>% 
  filter(Constituent != "Not coded") %>% 
...
ggplot() + 
  ...
  labs(x = "", 
       y = "", 
       fill = "Total Contacts",
       title = paste("Total Number of Contacts from members of the", Chamber)) +
  scale_fill_viridis_c(option = "C", end = .8) +
...
```{r members_barcoded, fig.height=4.4, fig.width=3}
...
```{r pac_amounts, fig.height= 3.7, fig.width=6.5}
library(tidyverse)
...
```{r, save letters_money, eval=FALSE}
...
```{r full-model-predicted, fig.height=3, fig.width=4.5}
mAll_Full %>% pplot()  + 
...
```{r full-model-predicted-pro, fig.height=3, fig.width=4.5}
mPro_Full %>% pplot()  + 
...
```{r full-model-predicted-anti, fig.height=3, fig.width=4.5}
mAnti_Full %>% pplot()  + 
...
```{r full-model-predicted-pro-anti, fig.height=6, fig.width=4.5}
p <- mPro_Full %>% pplot2() + 
  labs(y = "Predicted Pro-business\nLetters to FERC")
...
```{r full-logit, fig.height=3, fig.width=4.5}
# full logit
...
```{r full-logit-pro, fig.height=3, fig.width=4.5}
# full logit pro
...
p <- mpro_Fulllogit %>% augment(type.predict = "response", se_fit = T, interval = "confidence") %>%
  ggplot() +
  aes(x = PAC_100k, y = .fitted, 
      ymax = .fitted + (1.96*.se.fit),
      ymin = .fitted - (1.96*.se.fit) ) +
...
```{r full-logit-anti, fig.height=3, fig.width=4.5}
# full anti logit
...
a <- manti_Fulllogit %>% augment(type.predict = "response", se_fit = T, interval = "confidence") %>% 
  ggplot() +
  aes(x = PAC_100k, y = .fitted, 
      ymax = .fitted + (1.96*.se.fit),
      ymin = .fitted - (1.96*.se.fit) ) +
...
```{r full-logit-pro-anti, fig.height=6, fig.width=4.5}
p / a
```
...
# with interaction with majoirty 
mAnti_pac_partyx <- glm(AntiBusiness_Letters ~ PAC_100k + party_name*Majority, 
                                  data = letters_money %>% 
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr,congress,PAC_100k, chamber,AntiBusiness_Letters, party_name, Majority) %>%
                      distinct(),
            family = poisson)

# tidy(mAnti_pac_party)
mplot(mAnti_pac_partyx)+ labs(x="", y="Additional anti-business letters per congress")



#PAC + party*status interaction:
mAnti_pacXparty <- glm(AntiBusiness_Letters ~ PAC_100k + party_name*Majority, 
                                  data = letters_money %>% 
                        filter(industry == "energy") %>% 
                        filter(party_name != "Independent") %>% 
                        select(icpsr, congress,chamber,PAC_100k, AntiBusiness_Letters, party_name, Majority) %>%
                      distinct(),
            family = poisson)
...
dplot(models)+ 
    labs(x = "Additional letters per hundred thousand dollars per cycle")
