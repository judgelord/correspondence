...
letters_money %<>% left_join(d %>% select(icpsryear, nominate.dim1, chair_since_2007)) %>% rename(chair = chair_since_2007)
...
# Full model:
mPro_Full<-glm(ProBusiness_Letters ~ PAC_contributions + nominate.dim1 + chair, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_Full)
mplot(mPro_Full)+
  labs(x="", y="Additional letters per congress")
```
...
knitr::kable(models %>% select(model, everything()), digets = 3, caption = "Additional letters per million dollars per cycle")
...
knitr::kable(models%>% select(model, everything()), digets = 3, caption = "Average additional letters per cycle per million dollars 1990-2014")
...
knitr::kable(models%>% select(model, everything()), digets = 3, caption = "Average additional letters per cycle")
```
