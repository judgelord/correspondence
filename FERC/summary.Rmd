...
So far we have matched `r nrow(d)` of these letters to member names for years `r unique(d$year)`. 
...
(click `CODE` to see inductively-developed coding rules):
```{r code, eval=FALSE}
d %<>%
  ...
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("PUBLIC UTILITIES COMMISSION", SUBJECT, ignore.case = TRUE), 
                             "1", CERTAINTY)) 
```
## Coders coding of constituent type 
To-do: share marked "Constituent" vs those with Pro or Anti Business
...
```{r}
d %<>% str_replace("RTO", "Regional Transmission Organization")
d %>%
  filter(!is.na(ProBusiness)) %>%
  group_by(ProBusiness) %>%
  select(id, last_name, ProBusiness) %>%
  distinct() %>%
