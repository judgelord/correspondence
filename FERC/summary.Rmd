...
```{r part-constituent, fig.height=2, fig.width=4}
d %>% 
  ...
  facet_grid(party_name ~ majority) + 
...
```{r part-business, fig.height=2, fig.width=4}
d %>% 
  ...
  count(party_name, position, majority) %>% 
  filter(party_name != "Independent",
         !is.na(position),
         !is.na(majority)) %>% 
  ggplot() + 
  aes(y = n, x = "", fill = position) + 
  geom_col( position = "dodge") + 
  facet_grid(party_name ~ majority)+ 
  scale_fill_viridis_d(option = "C", end = .8) + 
  labs(x = "",
...
```{r constituent-non-constituent}
d %<>% 
  mutate(letter_position = ifelse(!is.na(ProBusiness)|!is.na(ProProject), "ProBusiness", NA),
...
d  %>% 
  drop_na(letter_position, Constituent) %>% 
  ggplot() + 
...
d  %>% 
  ...
  geom_bar() + 
  facet_wrap("Constituent") +
  labs(title = "Total coded letters", 
       x = "",
       y = paste("Letters 2000-2018, N =", as.character(d %>% nrow() %>% as.character() )))
```
...
m <- glm(Non_Constituent_Letters ~ PAC_contributions, data = Non_Constituent_money, family = poisson  ) 
tidy(m)
m %>% mplot()+
  labs(x="", y="Additional letters per congress per million dollars")
```
