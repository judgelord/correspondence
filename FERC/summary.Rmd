...
# Full model:
mPro_Full <- glm(ProBusiness_Letters ~ PAC_contributions + nominate.dim1 + chair + oversight_committee, data = letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_Full)
mplot(mPro_Full)+
  labs(x="", y="Additional letters per congress")
```
...
library(stargazer)
library(dplyr)
write_lines(stargazer(mAll, mPro, mAnti, mNonConstituent),
