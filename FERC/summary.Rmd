...
dpacs %<>% 
  ...
  select(ID, name_state, position, company_short, pacs, comid, PAC_contributions) %>% 
  distinct()
  
dpacs %>% 
  group_by(name_state, position, company_short, pacs, comid) %>% 
  summarise(PAC_contributions = sum(PAC_contributions, na.rm = T)) %>% 
  ungroup() %>% 
  group_by(position)  %>%
