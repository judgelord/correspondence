...
dcommittees %>%  
  ...
  dplyr::select(committee, position, chair, chair_since_2007, DATE, assigneddate, terminationdate) %>% 
  distinct() %>% 
...
Non_Constituent_money <- d %>% 
  ...
  #filter(Type %in% c("Corp. Constituent", "Policy")) %>% 
  group_by(icpsryear, cycle, name_state, party, congress, icpsr) %>%
  summarise(Non_Constituent_Letters = n())
...
# Add zero-letter-cycle observations:
Non_Constituent_money %<>% 
 mutate(Non_Constituent_Letters = ifelse(is.na(Non_Constituent_Letters) & cycle > 1997, 0, Non_Constituent_Letters)) 
...
letters_money %<>% left_join(d %>%
                               dplyr::select(icpsryear, nominate.dim1, chair_since_2007, oversight_committee, chamber) %>%
  dplyr::rename(chair = chair_since_2007))
...
mplot(mPro_pac_nominate)+
  labs(x="", y="Additional letters per congress")
```

#### Full (multivariate) models 
```{r full-model}
# MERGE IN LETTER DATA TO ALL Letters COUNTS  
All_Letters_money %<>% left_join(d %>% 
dplyr::select(icpsryear, nominate.dim1, chair_since_2007, oversight_committee, chamber) %>%
  dplyr::rename(chair = chair_since_2007))

# Full model, all letters
mAll_Full <- glm(All_Letters ~ PAC_contributions + party + chair + oversight_committee + chamber + chamber, data = All_Letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
mAll_Fullx <- glm(All_Letters ~ PAC_contributions*party + chair + oversight_committee + chamber, data = All_Letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
mPro_Full <- glm(ProBusiness_Letters ~ PAC_contributions + party + chair + oversight_committee + chamber, data = letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
# Full model with interaction:
mPro_Fullx <- glm(ProBusiness_Letters ~ PAC_contributions*party + chair + oversight_committee + chamber, data = letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
# MERGE IN LETTER DATA TO ANTI 
Antiletters_money %<>% left_join(d %>% 
dplyr::select(icpsryear, nominate.dim1, chair_since_2007, oversight_committee, chamber) %>%
  dplyr::rename(chair = chair_since_2007))


mAnti_Full <- glm(AntiBusiness_Letters ~ PAC_contributions + party + chair + oversight_committee + chamber, data = Antiletters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
# full model with interaction 
mAnti_Fullx <- glm(AntiBusiness_Letters ~ PAC_contributions*party + chair + oversight_committee + chamber, data = Antiletters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mAnti_Fullx)
mplot(mAnti_Fullx)+
  labs(x="", y="Additional letters per congress")
...
Antiletters_money %<>% 
  drop_na(AntiBusiness_Letters, PAC_contributions) %>% mutate(Log_PAC_contributions = log(PAC_contributions+1)) 
...
models <- tidy(mAll_Full) %>% 
  mutate(model = "Letters per Cycle") %>%
