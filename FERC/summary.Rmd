...
requires <- c("tidyr", "broom", "knitr","ineq", "dplyr", "ggplot2", "magrittr", "stargazer", "maps", "fiftystater", "mapproj", "dotwhisker", "here", "pander", "memisc")
...
dcommittees %>%  
  ...
  select(committee, position, chair, chair_since_2007, DATE, assigneddate, terminationdate) %>% 
  distinct() %>% 
  filter(str_detect(committee, "ENERGY$|ENVIRONMENT|NATURAL")) %>% 
  mutate(position = ifelse(position == "Other", NA, position)) %>%
  filter(# !is.na(position),
         chair_since_2007 == T) %>%
  select(DATE, chair, position, assigneddate, terminationdate, committee) %>% 
  distinct() %>% 
  ggplot() +
  geom_point(
    aes(x = DATE, 
...
mProParty <- glm(ProBusiness_Letters ~ party, data = letters_money %>% filter(party !="NA"), family = "poisson" ) 
...
# PAC + party:
mPro_pac_party<-glm(ProBusiness_Letters ~ PAC_contributions + party, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_pac_party)
mplot(mPro_pac_party)+
  labs(x="", y="Additional letters per congress")

# PAC*party interaction:
mPro_pacXparty<-glm(ProBusiness_Letters ~ PAC_contributions * party, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_pacXparty)
mplot(mPro_pacXparty)+
  labs(x="", y="Additional letters per congress")

letters_money %<>% left_join(d %>% select(icpsryear, nominate.dim1, chair_since_2007, oversight_committee)) %>% rename(chair = chair_since_2007)

# DW NOMINATE 
mPro_nominate <-glm(ProBusiness_Letters ~  nominate.dim1, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_nominate)
mplot(mPro_nominate)+
  labs(x="", y="Additional letters per congress")

# PAC + ideology:
mPro_pac_nominate <-glm(ProBusiness_Letters ~ PAC_contributions * nominate.dim1, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_pac_nominate)
mplot(mPro_pac_nominate)+
  labs(x="", y="Additional letters per congress")

# Full model:
mPro_Full <- glm(ProBusiness_Letters ~ PAC_contributions + nominate.dim1 + chair, oversight_committee, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mPro_Full)
mplot(mPro_Full)+
  labs(x="", y="Additional letters per congress")
```
...
mAntiParty <- glm(AntiBusiness_Letters ~ party, data = Antiletters_money %>% filter(party !="NA"), family = "poisson" ) 
...
knitr::kable(models %>% dplyr::select(model, everything()), digets = 3, caption = "Additional letters per million dollars per cycle")

write_lines(
  stargazer::stargazer(mAll, mPro, mAnti, mNonConstituent),
...
knitr::kable(models%>% dplyr::select(model, everything()), digets = 3, caption = "Average additional letters per cycle per million dollars 1990-2014")

write_lines(
  stargazer::stargazer(mProTotal, mAntiTotal),
...
knitr::kable(models%>% dplyr::select(model, everything()), digets = 3, caption = "Average additional letters per cycle")
...
write_lines(
  stargazer::stargazer(mAll, mPro, mAnti,mNonConstituent, mProLog, mAntiLog, mProParty, mProTotal, mAntiTotal, mAntiParty),
  path = here("FERC/tables/m.tex"))
```
