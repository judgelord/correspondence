...
filter(dcorps, company_short %in% topcorps$company_short) %>% select(company_short, company_short_refs, ID) %>% distinct() %>% knitr::kable() 

# corps with short names 
shortcorps <- dcorps %>% 
  group_by(company_short) %>% 
  tally() %>% arrange(nchar(company_short)) %>% 
  mutate(nchar = nchar(company_short))%>%
  top_n(30, -nchar) 

filter(dcorps, company_short %in% shortcorps$company_short) %>% select(company_short, company_short_refs) %>% distinct() %>% knitr::kable()
