...
Using CRP industry codes for PACs and DIME contribution matrix, sum up sector-level contributions by memeber.
```{r}
load(here("data/contrib_total.rds"))
contrib_total %<>% 
  rename(icpsryear = ICPSR,
         PAC_contributions = value)
head(contrib_total$icpsryear)
...
letters_money <- d %>% 
  ...
  group_by(icpsryear, bioname) %>%
  summarise(Letters = n() ) %>% 
  # join by ICPSR, adding a sum of sector PAC contributions per member

  left_join(contrib_total) 
```

Linear model: Letters per cycle ~ Energy Sector PAC contributions per cycle 2000-2016
```{r money-letters-per-cycle}
lm(Letters ~ PAC_contributions, data = letters_money) %>% 
  tidy()
```

Linear model: Total letters ~ total Energy Sector PAC contributions 2000-2016
