...
PACs <- contrib_matrix %>% select(comid, UltOrg,Affiliate,PACShort) %>% distinct()

# Test corp that we know has a PAC
corp <- c("ONEOK")

# A function to match PACs with companies in the letters data
get_pacs <- function(corps, pacs){
ifelse(str_detect(pacs, corps), pacs, NA)
}

map2(corp, PACs$UltOrg, get_pacs) %>% 
  discard(is.na) %>% 
  # split out multiple ultorgs per pac
  str_split(";") %>% 
  # select unique ones
  unlist() %>% unique() %>% 
...
dcorp %>% 
  mutate(pac = pmap_chr(company_short, ) )
