...
dcorps %>% 
  ...
  select(ID, company_short, company, SUBJECT)%>% 
  group_by(company_short) %>% 
  distinct() %>%
  add_count() %>% 
  arrange(-n) %>% 
...
```{r add-zeros2, include = FALSE}
letters_money %<>% 
 mutate(ProBusiness_Letters = ifelse(is.na(ProBusiness_Letters) & cycle > 2000, 0, ProBusiness_Letters)) 
```
...
ggplot() + 
  ...
  labs(x="", y="OLS Estimate, additional letters per dollar")}

lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money) %>% mplot()
```

Subset to members who wrote at least once:
```{r money-letters-per-cycle-model-no-zeros, fig.height=1}
lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money %>% filter(ProBusiness_Letters>0))  %>% mplot()
```

With a party interaction:
```{r money-letters-per-cycle-model-party, fig.height=1}
lm(ProBusiness_Letters ~ PAC_contributions*party, data = letters_money %>% filter(party !="NA"))  %>% mplot()
```
...
lm(ProBusiness_Letters_Per_Cycle ~ PAC_contributions, data = letters_money_total)  %>% mplot()
```
...
```{r add-zeros, include = FALSE, eval = FALSE}
source(here("setup.R"))
