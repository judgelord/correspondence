...
# Drop non company observations from d 
...
letters_money %>% filter(!is.na(name_state)) %>% select(-icpsryear, -icpsr) %>% top_n(10, PAC_contributions) %>% knitr::kable()
```
...
letters_money_total <- letters_money %>% 
  ...
  summarise(ProBusiness_Letters_Per_Cycle = round(mean(ProBusiness_Letters, na.rm = TRUE)),
...
### Linear model: Pro-Business Letters per cycle ~ Energy Sector PAC contributions per cycle 1998-2014
...
ggplot() + 
  ...
  labs(x="", y="OLS Estimate, additional letters per thousand dollars")}

lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money %>% mutate(PAC_contributions = PAC_contributions/1000) ) %>% mplot()
```

Subset to members who wrote at least once:
```{r money-letters-per-cycle-model-no-zeros, fig.height=1}
lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money %>% filter(ProBusiness_Letters>0) %>% mutate(PAC_contributions = PAC_contributions/1000) )  %>% mplot()
```

By party:
```{r letters-per-cycle-model-party, fig.height=1}
m <- lm(ProBusiness_Letters ~ party, data = letters_money %>% filter(party !="NA") ) %>% tidy()

m
```
Republicans send an additional `r round(m$estimate[2],1)` letters per Congress, on average.



PAC*party interaction:
```{r money-letters-per-cycle-model-party, fig.height=1}
lm(ProBusiness_Letters ~ PAC_contributions*party, data = letters_money %>% filter(party !="NA")%>% mutate(PAC_contributions = PAC_contributions/1000) )  %>% mplot()
```



### Linear model: Average Pro-Business letters per cycle ~ total Energy Sector PAC contributions 1990-2014
```{r money-letters-total, fig.height=1}
lm(ProBusiness_Letters_Per_Cycle ~ PAC_contributions, data = letters_money_total%>% mutate(PAC_contributions = PAC_contributions/1000))  %>% mplot()
```

Subset to members who wrote at least once:
```{r money-letters-total-no-zeros, fig.height=1}
lm(ProBusiness_Letters_Per_Cycle ~ PAC_contributions, data = letters_money_total %>% filter(ProBusiness_Letters_Per_Cycle>0))  %>% mplot()
```
