...
# top corps
topcorps <- dcorps %>% group_by(company_short) %>% tally() %>% arrange(-n) %>% top_n(30)
...
letters_money <- d %>% 
  ...
  left_join(contrib_total) %>% ungroup()

letters_money %>% top_n(10, PAC_contributions) %>% knitr::kable()
```
...
```{r money-letters-per-cycle}
lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money) %>%
  tidy() %>% knitr::kable()
```

Linear model: Total Pro-Business letters ~ total Energy Sector PAC contributions 2000-2016
```{r money-letters-total}
lm(ProBusiness_Letters ~ PAC_contributions, data = letters_money_total) %>% 
  tidy() %>% knitr::kable()
```
