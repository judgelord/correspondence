...
Antiletters_money <- d %>%
  ...
  #filter(Type %in% c("Corp. Constituent", "Policy")) %>% 
  group_by(icpsryear, cycle, name_state, party, congress, icpsr) %>% distinct() %>% 
...
dplyr::select(icpsryear, nominate.dim1, #chair_since_2007,
              oversight_committee, chamber) %>% distinct() #%>% dplyr::rename(chair_since_2000 = chair_since_2007)
) %>% distinct()
nrow(letters_money)
...
mAll_Full <- glm(All_Letters ~ PAC_contributions + party +  oversight_committee + chamber, data = All_Letters_money %>% 
...
mAll_Fullx <- glm(All_Letters ~ PAC_contributions*party + oversight_committee + chamber, data = All_Letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
mPro_Full <- glm(ProBusiness_Letters ~ PAC_contributions + party +  oversight_committee + chamber, data = letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
# Full model with interaction:
mPro_Fullx <- glm(ProBusiness_Letters ~ PAC_contributions*party + oversight_committee + chamber, data = letters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
mAnti_Full <- glm(AntiBusiness_Letters ~ PAC_contributions + party +  oversight_committee + chamber, data = Antiletters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
...
# full model with interaction 
mAnti_Fullx <- glm(AntiBusiness_Letters ~ PAC_contributions*party +  oversight_committee + chamber, data = Antiletters_money %>% filter(!party %in% c("NA","(I)", "Independent"))%>% mutate(PAC_contributions = PAC_contributions/1000000), family = "poisson" )  
tidy(mAnti_Fullx)
mplot(mAnti_Fullx)+
  labs(x="", y="Additional letters per congress")
```
