...
trim <- function(d){
d %>%
  str_remove_all("Village of |\\(.*|,.*| -.*| Pipe.*| Company.*| Corp.*| Energy.*| Holdings.*| Gas$| Project.*| LLC| L.L.C.|  L.P.| Inc.| L.P.| Ltd.| LP| Limited Partnership| Limited Liability| Transportation and Storage| Storage and Transportation") %>% 
  str_remove_all("^, ") %>%
...
dcorps %<>% 
  ...
  mutate(company_short = ifelse(company_short %in% c("N/A|Na|NO|Clean Water Act"), "NOT A COMPANY?", company_short))
