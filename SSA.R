...
clean <- function(file.name) {
  ...
  mutate(TYPE = ifelse (!grepl("[0-9]", TYPE) & grepl("UNDERPAYMENT|INITIAL CLAIM|CITIZENSHIP|OVERPAYMENT|CONSTITUENT|DISABILITY|DENIED|STATUS ON CLAIM|RETIREMENT|WIDOW|BILLING|HIRING PRACTICE|WITHDRAWAL|2ND REQUEST|SUICIDE|INITIAL AWARD|ATTORNEY FEE|ADDITIONAL PAYMENT", SUBJECT, ignore.case = TRUE), "1", TYPE)) %>%
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("UNDERPAYMENT|INITIAL CLAIM|CITIZENSHIP|OVERPAYMENT|CONSTITUENT|DISABILITY|DENIED|STATUS ON CLAIM|RETIREMENT|WIDOW|BILLING|HIRING PRACTICE|WITHDRAWAL|2ND REQUEST|SUICIDE|INITIAL AWARD|ATTORNEY FEE|ADDITIONAL PAYMENT", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
  mutate(TYPE = ifelse (!grepl("[0-9]", TYPE) & grepl("FIRST DRAFT|HEARING|RULE|ADCLCA TO CONTROL|REQUEST FOR INFORMATION|ALJ DECISION|RECONSIDERATION DETERMINATION", SUBJECT, ignore.case = TRUE), "5", TYPE)) %>%
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("FIRST DRAFT|HEARING|RULE|ADCLCA TO CONTROL|REQUEST FOR INFORMATION|ALJ DECISION|RECONSIDERATION DETERMINATION", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
  ...
  mutate(TYPE = ifelse (!grepl("[0-9]", TYPE) & grepl("CLAIM|BENEFITS|COMPLAINT|APPEAL PENDING|STAFFER", SUBJECT, ignore.case = TRUE), "1", TYPE)) %>%
  mutate(CERTAINTY = ifelse (!grepl("[0-9]", CERTAINTY) & grepl("CLAIM|BENEFITS|COMPLAINT|APPEAL PENDING|STAFFER", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
  mutate(POLICY_EVENT = ifelse (!grepl("[0-9]", POLICY_EVENT) & grepl("RULE", SUBJECT, ignore.case = TRUE), "RULE", POLICY_EVENT)) %>%
