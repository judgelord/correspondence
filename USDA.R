...
data %<>%
  ...
  mutate(CERTAINTY = ifelse (grepl("TRAVEL", SUBJECT, ignore.case = TRUE), "1", CERTAINTY)) %>%
  mutate(TYPE = ifelse (grepl("PERSONNEL", SUBJECT, ignore.case = TRUE), "6", TYPE)) %>%   #NOT QUITE SURE WHAT IS MEANT BY "PERSONNEL" BUT IT SHOWS UP A LOT
  mutate(CERTAINTY = ifelse (grepl("PERSONNEL", SUBJECT, ignore.case = TRUE), "3", CERTAINTY)) %>%
