...
data %<>%
  ...
  data %<>%
    ...
    mutate(ERROR = ifelse(str_detect(FROM, "Boyle, Kevin|Briggs, Tim|Duff, Bob") & is.na(last_name), "state legislator", ERROR)) %>%
    ...
    mutate(ERROR = ifelse(str_detect(FROM , "Graham, Bob") & congress %in% 111, "no longer in congress", ERROR))
