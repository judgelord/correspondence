...
fixproject <- data %>% 
  ...
  filter(!grepl("on behalf .* constituent", SUBJECT, ignore.case = TRUE), grepl("Cove Point LNG project", SUBJECT, ignore.case = TRUE), is.na(ProProject), is.na(ProBusiness), is.na(AntiBusiness), is.na(AntiProject))
