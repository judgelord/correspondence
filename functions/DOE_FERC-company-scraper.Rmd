---
#title: "Tidy text analysis example: web scraping and RegEx with `rvest`, `stringr`, and `purrr`"
...
- [`purrr` cheatsheet](https://www.rstudio.com/resources/cheatsheets/#purrr) (applying functions)
...
html %>% 
  html_nodes("strong") %>% # company names appear in the HTML tag "<strong>")
...
FERC_corps %<>% 
  ...
  filter(!str_detect(company, "[0-9]{2}/[0-9]{2}/[0-9]{4}")) %>% 
...
d%>%
  ...
  str_remove_all(",.*| Pipe.*| Company.*| Corp.*|  Co.| Gen.| LLC| L.L.C.|  L.P.| Inc.| L.P.| Ltd.| \\(.*") %>% 
  str_remove_all(" Energy.*| Holdings.*| Gas$| Project.*| -.*|Village of |\\(.*")
}

FERC_corps$company_short <- trim(FERC_corps$company)
head(FERC_corps)
```
...
## Find corporation names in letter summary
FERC_letters$corp <- str_extract_all(FERC_letters$summary, ferc_short, simplify = TRUE) %>% apply(1, str_unique) %>% str_remove_all("; ; |; $")
...
FERC_letters$company_short <- str_extract_all(FERC_letters$text, ferc_short, simplify = TRUE) %>% map(str_unique) %>% str_remove_all("; ; |; $")
...
## Individual contributors at Fortune 500 companies.
...
## Fortune 500 indicator to 
FERC_corps %<>% mutate(Fortune500_short = str_extract(fortune500, company_short) )

# DIME records that exactly match a FERC company name
dime_1 <- dime500 %>% filter(corpname %in% unique(ferc$f500) )
unique(dime_1$corpname)

# DIME records that match a shortened version of a FERC company name
dime_2 <- dime500 %>% filter(str_detect(corpname, ferc_short))
unique(dime_2$corpname)

# Join the two DIME subsets
dime500_ferc <- full_join(dime_1, dime_2)
save(FERC_letters, file = here("data", "FERC_letters_corps.Rdata"))
...
FERC_letters <- FERC_letters %>% mutate(Fortune500 = str_detect(corp, dime500_ferc_corps))
...
## Match all companies from the FERC website and congressional letters to DIME
```{r}
...
## Match individual campaign donations
