...
In addition to `tidyverse` packages, this example uses the `magrittr`, `pdftools`, and `here` packages.
...
The URLs themselves are in the "href" attribute, which we can extract with `html_attr()`. However, there is a problem; they have no file extentions. Luckily, the linked text says what kind of file it is, so we can add a ".pdf" or ".tif" extension, depending on whether the linked text contains "PDF" or "Image" using `mutate()` and `str_replace`.
```{r}
  urls <- tibble(
...
# Apply this function extract the table and download all files linked on each page
```{r}
