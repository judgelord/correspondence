...
constituent_types %<>% 
  select(agency, keywords, constituent_type, constituent_class, TYPE) %>% 
  mutate_all(blank_na)
...
#FIXME WITH PURRR
add_constituent_type <- function(d, keyword, constituent_type, constituent_class, type, Agency){
  
  d %<>% mutate(CONSTITUENT_TYPE = ifelse(str_detect(str_to_lower(SUBJECT), keyword) & agency %in% Agency , # if keword in subject
                                          paste(constituent_type, CONSTITUENT_TYPE, sep = ";"), # add constituent_type
                                          CONSTITUENT_TYPE) %>% 
                  str_remove_all(";NA|;NULL|^NULL|^;")) 

  d %<>% mutate(CONSTITUENT_CLASS = ifelse(str_detect(str_to_lower(SUBJECT), keyword) & agency %in% Agency , # if keword in subject
