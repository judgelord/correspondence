...
add_first <- function(data){
  ...
  if(!"first_name" %in% names(data)){
    ...
    # format last name to match members
    data$last_name <- formatLastName(data, "last_name")
    
    data %<>%
      left_join(membersOneLastName)
    return(data)
  } else{
...
extractMemberName_old <- function(data, members = members, col_name, congresses = unique(data$congress)){
