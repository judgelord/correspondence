...
members <- full_join(member_search(congress = c(105:108)) %>% select(-congresses),
 ...
 members%<>%
  ...
  # middle name
    ...
    mutate(middle_name = ifelse(bioname == "KLEIN, Ron", "Paul", middle_name)) %>%
    mutate(middle_name = ifelse(bioname == "DUFFY, Sean", "Patrick", middle_name)) %>%
    mutate(middle_name = ifelse(bioname == "MURPHY, Patrick" & str_detect(state, "florida"), "Erin", middle_name)) %>%
